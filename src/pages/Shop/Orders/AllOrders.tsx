...
const AllOrders = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}api/shop/get-all-orders/?search=${encodeURIComponent(
          search,
        )}&page=${page}`,
        {
          headers: {
  ...
  }, [baseUrl, search, page, draft, userToken]);
  ...
  const handleDelete = async (itemId) => {
    const data = { order_id: itemId };

    try {
      const response = await fetch(`${baseUrl}api/orders/delete-order/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Token ${userToken}`,
        ...
        ? orders.map((order) => (
            ...
            >
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                    <div className="h-12 w-12 overflow-hidden rounded-md">
                      <img
                        src={`${baseUrlMedia}${order.product_image}`}
                        alt="Orders"
                  ...
                  <p className="text-sm text-black dark:text-white">
                    {`${truncateText(order.order_id, 50)}`}
                  </p>
                </div>
              </div>
              ...
              <div className="col-span-2 hidden items-center sm:flex">
                <p className="text-sm text-black dark:text-white">
                  {truncateText(order.product_name, 50)}{' '}
                ...
                <p className="text-sm text-black dark:text-white">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => openDeleteModal(order.order_id)} // Pass the ID of the item to be deleted
                    >
                      <svg
                        className="fill-current"
                        width="18"
