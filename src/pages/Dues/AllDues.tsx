...
const AllDues = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}api/dues/admin/get-all-dues-entries/?search=${encodeURIComponent(
          search,
      ...
      setDuess(data.data.dues_entries);
      setTotalPages(data.data.pagination.total_pages);
      console.log('Total Pages:', data.data.pagination.total_pages);
      console.log(data.data.dues_entries);
    } catch (error) {
  ...
  const handleDelete = async (itemId) => {
    const data = { dues_id: itemId };

    try {
      const response = await fetch(`${baseUrl}api/dues/delete-dues/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Token ${userToken}`,
                ...
                <p className="text-sm text-black dark:text-white">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => openDeleteModal(dues.dues_id)} // Pass the ID of the item to be deleted
                    >
                      <svg
                        className="fill-current"
                        width="18"
