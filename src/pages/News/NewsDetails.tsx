...
const NewsDetails = () => {
  ...
  const [newsImages, setEventImages] = useState([]);
  const [newsVideos, setEventVideos] = useState([]);
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          baseUrl + `api/news/get-news-details/?news_id=${news_id}`,
          {
            headers: {
        ...
        setEvent(data.data);
        setEventImages(data.data.news_images);
        setEventVideos(data.data.news_videos);
      } catch (error) {
  ...
  const handlePublish = async (e) => {
    ...
    const url = baseUrl + 'api/news/post-news/';
    const data = {
      'news_id': news_id,
    };
      ...
      <div className="grid grid-cols-2 gap-8">
        <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <div className="relative z-20 h-35 md:h-65">
            <img
              src={`${baseUrlMedia}/media/${newsImages[0]}`}
              alt="photo cover"
  ...
  <div className="flex gap-2 absolute bottom-1 left-1 z-10 xsm:bottom-4 xsm:left-4">
              ...
              >
                ...
                <span>{ news.draft === true ? 'Publish' : 'Draft'}</span>
              </label>
...
<Link to={'/add-news-images/' + news.news_id}>
<label
              htmlFor="cover"
              className="flex cursor-pointer items-center justify-center gap-2 rounded bg-primary py-1 px-2 text-sm font-medium text-white hover:bg-opacity-90 xsm:px-4"
            >
          ...
          <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
            <div className="mt-4">
              ...
              <p className="font-medium">{news.content || 'N/A'}</p>
              ...
              <div className="grid grid-cols-2 ">
                <div className="mx-auto max-w-180 mt-3">
                  ...
                  <p className="mt-4">{news.published_at || 'N/A'}</p>
                </div>
                ...
                <div className="mx-auto max-w-180 mt-3">
                  ...
                  <p className="mt-4">{news.read_duration || 'N/A'}</p>
                </div>
              </div>
