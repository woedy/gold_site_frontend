...
const AllNews = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}api/news/admin/get-all-news/?search=${encodeURIComponent(
          search,
  ...
  const handleDelete = async (itemId) => {
    const data = { news_id: itemId };

    try {
      const response = await fetch(`${baseUrl}api/news/delete-news/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Token ${userToken}`,
                ...
                <p className="text-sm text-black dark:text-white">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => openDeleteModal(news.news_id)} // Pass the ID of the item to be deleted
                    >
                      <svg
                        className="fill-current"
                        width="18"
