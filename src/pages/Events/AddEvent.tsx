...
const AddEvent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (eventTitle === '') {
      setInputError('Event title required.');
      return;
    }

    if (eventTheme === '') {
      setInputError('Event Theme required.');
      return;
    }


    if (eventSubject === '') {
      setInputError('Event Subject required.');
      return;
    }

    if (eventDate === '') {
      setInputError('Event Date required.');
      return;
    }

    if (eventTime === '') {
      setInputError('Event Time required.');
      return;
    }

    if (eventVenue === '') {
      setInputError('Event Venue required.');
      return;
    }

    if (eventOrganizer === '') {
      setInputError('Event Organizer required.');
      return;
    }
    ...
    formData.append('subject', eventSubject);
    formData.append('event_date', eventDate);
    formData.append('event_time', eventTime);
    formData.append('venue', eventVenue);
    formData.append('organised_by', eventOrganizer);

    // Make a POST request to the server
    const url = baseUrl + 'api/events/add-event/';
