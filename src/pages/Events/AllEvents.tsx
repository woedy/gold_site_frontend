...
const AllEvents = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}api/events/admin/get-all-events/?search=${encodeURIComponent(
          search,
  ...
  const handleDelete = async (itemId) => {
    const data = { event_id: itemId };

    try {
      const response = await fetch(`${baseUrl}api/events/delete-event/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Token ${userToken}`,
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                    <div className="h-12 w-12 overflow-hidden rounded-md">
                      <img
                        src={`${baseUrlMedia}${event.event_image}`}
                        alt="Event"
                ...
                <p className="text-sm text-black dark:text-white">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => openDeleteModal(event.event_id)} // Pass the ID of the item to be deleted
                    >
                      <svg
                        className="fill-current"
                        width="18"
